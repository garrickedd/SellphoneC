...
namespace SellphoneC.Services
{
    public class DatabaseInitializer
    {
            ...
            RoleManager<IdentityRole>? roleManager)
        {
            ...
            if (!exists)
            {
                Console.WriteLine("Seller role is not defined and will be create");
            ...
            if (!exists) 
            {
                Console.WriteLine("Client role is not defined and will be create");
                await roleManager.CreateAsync(new IdentityRole("client"));
            }

            // check if we have at lease one admin user or not
